...
macro "Add morphological centroid coordinates to Results Table" {
	...
	for (i=0 ; i<roiOriginalCount; i++) {
		...
		newImage("Contained Points","8-bit black",Rwidth,Rheight,1);
		...
		if (lcf==1) {
			for (RPx=1; RPx<(Rwidth-1); RPx++){
				for (RPy=1; RPy<(Rheight-1); RPy++){ // start at "1" because there should not be a pixel at the border
					if((getPixel(RPx, RPy))==255) {  
						...
						setResult("mc_offsetX", i, getResult("X",i)-(RPx+Rx));
						setResult("mc_offsetY", i, getResult("Y",i)-(RPy+Ry));
						RPy = Rheight;
			...
			for (RPx=1; RPx<(Rwidth-1); RPx++){
				for (RPy=1; RPy<(Rheight-1); RPy++){ // start at "1" because there should not be a pixel at the border
					if((getPixel(RPx, RPy))==255) {
						setResult("mc_X\(px\)", i, round((RPx+Rx)/lcf));
						setResult("mc_Y\(px\)", i, round((RPy+Ry)/lcf));					
						setResult("mc_X\(" + selectedUnit + "\)", i, lcf*(RPx+Rx));
						setResult("mc_Y\(" + selectedUnit + "\)", i, lcf*(RPy+Ry));
						setResult("mc_offsetX", i, lcf*(getResult("X",i)-(RPx+Rx)));
						setResult("mc_offsetY", i, lcf*(getResult("Y",i)-(RPy+Ry)));
						RPy = Rheight;
						RPx = Rwidth; // one point and done
					}
				}
