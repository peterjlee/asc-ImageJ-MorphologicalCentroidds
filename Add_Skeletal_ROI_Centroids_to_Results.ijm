/* 	Based on "MCentroids.txt" Morphological centroids by G. Landini
	...
	v180602 changed name to coordinates
*/
		...
		for (RPx=1; RPx<(Rwidth-1); RPx++){
			for (RPy=1; RPy<(Rheight-1); RPy++){ /* start at "1" because there should not be a pixel at the border */
				if((getPixel(RPx, RPy))==0) {  
					setResult("skelc_X\(coord\)", i, RPx+Rx+0.5); /* add 0.5 to convert coordinated to pixel center */
					setResult("skelc_Y\(coord\)", i, RPy+Ry+0.5);
					// if (lcf!=1) {
...
/*-----------------functions---------------------*/
	...
	function binaryCheck(windowTitle) { /* For black objects on a white background */
		...
		if (((getPixel(0, 0))==0 || (getPixel(0, 1))==0 || (getPixel(1, 0))==0 || (getPixel(1, 1))==0)) {
			inversion = getBoolean("Background appears to be black, do you want it inverted?", "Yes Please", "No Thanks");
