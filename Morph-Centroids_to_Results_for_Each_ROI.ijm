/* 	Based on "MCentroids.txt" Morphological centroids by thinning assumes white particles: G. Landini
	...
	v180404 Checks to see if MCentroids already exist.
...
macro "Add morphological centroid coordinates to Results Table" {  
	...
	if (!isNaN(getResult('mc_X\(px\)',0))) {
		overwriteResults = getBoolean("Morphological centroids already exist in Results table; do you want to continue?");
		if (!overwriteResults) exit("Goodbye");
	}
	...
	for (i=0 ; i<roiOriginalCount; i++) {
		...
		if(addRadii) {
			...
			getSelectionCoordinates(xPoints, yPoints); /* place border coordinates in array for radius measurements - Wayne Rasband: http://imagej.1557.x6.nabble.com/List-all-pixel-coordinates-in-ROI-td3705127.html */
		}
